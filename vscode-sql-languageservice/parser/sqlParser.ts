 ...
 *--------------------------------------------------------------------------------------------*/
'use strict';
import { TokenType, Scanner, IToken } from './sqlScanner';
import * as nodes from './sqlNodes';
import { ParseError, SQLIssueType } from './sqlErrors';
...
export class Parser {
  ...
  public peekIdent(text: string): boolean {
    return (
      TokenType.Ident === this.token.type &&
  ...
  public peekKeyword(text: string): boolean {
    return (
      TokenType.AtKeyword === this.token.type &&
      text.length === this.token.text.length &&
      text === this.token.text.toLowerCase()
    );
  }
  ...
  public _parseSupportsDeclaration(isNested = false): nodes.Node {
    if (isNested) {
      ...
      return this._tryParseRuleset(isNested) || this._tryToParseDeclaration() || this._parseStylesheetStatement();
    }
  ...
  public _parseTerm(): nodes.Term {
    ...
    if (
      node.setExpression(this._parseURILiteral()) || // url before function
