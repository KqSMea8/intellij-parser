...
import { LintSettings } from '../sqlLanguageService';
...
export let Rules = {
  ...
  IncludeStandardPropertyWhenUsingVendorPrefix: new Rule(
    'vendorPrefix',
    localize('rule.standardvendorprefix.all', 'When using a vendor-specific prefix also include the standard property'),
    Warning
  ),
  DuplicateDeclarations: new Rule(
    'duplicateProperties',
    localize('rule.duplicateDeclarations', 'Do not use duplicate style definitions'),
    Ignore
  ),
  EmptyRuleSet: new Rule('emptyRules', localize('rule.emptyRuleSets', 'Do not use empty rulesets'), Warning),
  ImportStatemement: new Rule(
    'importStatement',
    localize('rule.importDirective', 'Import statements do not load in parallel'),
    Ignore
  ),
  BewareOfBoxModelSize: new Rule(
    'boxModel',
    localize('rule.bewareOfBoxModelSize', 'Do not use width or height when using padding or border'),
    Ignore
  ),
  UniversalSelector: new Rule(
    'universalSelector',
    localize('rule.universalSelector', 'The universal selector (*) is known to be slow'),
    Ignore
  ),
  ...
  RequiredPropertiesForFontFace: new Rule(
    'fontFaceProperties',
    localize('rule.fontFaceProperties', "@font-face rule must define 'src' and 'font-family' properties"),
    Warning
  ),
  HexColorLength: new Rule(
    'hexColorLength',
    localize('rule.hexColor', 'Hex colors must consist of three, four, six or eight hex numbers'),
    Error
  ),
  ArgsInColorFunction: new Rule(
    'argumentsInColorFunction',
    localize('rule.colorFunction', 'Invalid number of parameters'),
    Error
  ),
  UnknownProperty: new Rule('unknownProperties', localize('rule.unknownProperty', 'Unknown property.'), Warning),
  IEStarHack: new Rule(
    'ieHack',
    localize('rule.ieHack', 'IE hacks are only necessary when supporting IE7 and older'),
    Ignore
  ),
  UnknownVendorSpecificProperty: new Rule(
    'unknownVendorSpecificProperties',
    localize('rule.unknownVendorSpecificProperty', 'Unknown vendor specific property.'),
    Ignore
  ),
  PropertyIgnoredDueToDisplay: new Rule(
    'propertyIgnoredDueToDisplay',
    localize('rule.propertyIgnoredDueToDisplay', 'Property is ignored due to the display.'),
    Warning
  ),
  AvoidImportant: new Rule(
    'important',
    localize(
      'rule.avoidImportant',
