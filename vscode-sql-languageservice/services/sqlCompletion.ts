...
import { Symbols, Symbol } from '../parser/sqlSymbolScope';
...
import { ICompletionParticipant } from '../sqlLanguageService';
...
export class SQLCompletion {
      ...
      for (let i = this.nodePath.length - 1; i >= 0; i--) {
        ...
        if (node instanceof nodes.Property) {
          ...
          this.getCompletionsForFunctionArgument(
            <nodes.FunctionArgument>node,
            <nodes.Function>node.getParent(),
            result
          );
  ...
  public getCSSWideKeywordProposals(
    entry: languageFacts.IEntry,
    existingNode: nodes.Node,
    result: CompletionList
  ): CompletionList {
  ...
  public getCompletionsForSelector(ruleSet: nodes.RuleSet, isNested: boolean, result: CompletionList): CompletionList {
    let existingNode = this.findInNodePath(
      nodes.NodeType.PseudoSelector,
      nodes.NodeType.IdentifierSelector,
      nodes.NodeType.ClassSelector,
      nodes.NodeType.ElementNameSelector
    );
    if (
      !existingNode &&
  ...
  public getCompletionsForVariableDeclaration(
  ...
  public getCompletionsForExpression(expression: nodes.Expression, result: CompletionList): CompletionList {
    if (expression.getParent() instanceof nodes.FunctionArgument) {
      this.getCompletionsForFunctionArgument(
        <nodes.FunctionArgument>expression.getParent(),
        <nodes.Function>expression.getParent().getParent(),
        result
      );
      return result;
    }
  ...
  public getCompletionsForFunctionArgument(
    arg: nodes.FunctionArgument,
    func: nodes.Function,
    result: CompletionList
  ): CompletionList {
  ...
  private makeTermProposal(symbol: Symbol, parameters: nodes.Nodelist, existingNode: nodes.Node): CompletionItem {
    ...
    const insertText =
      symbol.name + '(' + params.map((p, index) => '${' + (index + 1) + ':' + p + '}').join(', ') + ')';
    return {
      label: symbol.name,
  ...
  public getCompletionsForSupportsCondition(
...
class ColorValueCollector implements nodes.IVisitor {
  ...
  public visitNode(node: nodes.Node): boolean {
    if (
      node instanceof nodes.HexColorValue ||
      (node instanceof nodes.Function && languageFacts.isColorConstructor(<nodes.Function>node))
    ) {
