(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "prettier", "fs-extra", "path", "./MetaGenerator", "../../common/utils", "../MetaLexer"], factory);
    }
    ...
    var MetaGenerator_1 = require("./MetaGenerator");
    ...
    var MetaParserConfig = (function () {
        function MetaParserConfig() {
            ...
            this.parserCodePath = '../../../grammars/ODPSSql/CoreParser.g4';
            this.lexerCodePath = '../../../grammars/ODPSSql/MysqlLexer.g4';
        }
        return MetaParserConfig;
    }());
    function metaLexerGenerator(lexerGCode, config) {
        var _a = MetaGenerator_1.parseGCode(lexerGCode, false), ast = _a.ast, cst = _a.cst, lexErrors = _a.lexErrors, parseErrors = _a.parseErrors;
        ...
        try {
            ...
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/ODPS/mysql_lexer_cst.g.json'), JSON.stringify(cst, null, 2));
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/ODPS/mysql_lexer_ast.g.json'), JSON.stringify(ast, null, 2));
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/ODPS/lexer.g.ts'), beautifiedCode);
        }
        catch (e) {
            console.error(e);
            debugger;
    ...
    function metaParserGenerator(parserGCode, config, tokens) {
        var _a = MetaGenerator_1.parseGCode(parserGCode, true, tokens), ast = _a.ast, cst = _a.cst, lexErrors = _a.lexErrors, parseErrors = _a.parseErrors;
        if (lexErrors && lexErrors.length) {
            console.log.apply(console, lexErrors);
            debugger;
        }
        ...
        var nodes = ast.rules.map(function (rule) {
            ...
            var subRuleNames = [];
            var tokenNames = [];
            MetaGenerator_1.traverse(rule, MetaLexer_1.TokenEnum.LowerName, function (node) {
                subRuleNames.push(node.name);
            });
            MetaGenerator_1.traverse(rule, MetaLexer_1.TokenEnum.UpperName, function (up) {
        ...
        try {
            var beautifiedCode = prettier.format(parserCode, config.prettierConfig);
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/ODPS/mysql_parser_cst.g.json'), JSON.stringify(cst, null, 2));
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/ODPS/mysql_parser_ast.g.json'), JSON.stringify(ast, null, 2));
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/ODPS/parser.g.ts'), beautifiedCode);
        }
        catch (e) {
            console.error(e);
            debugger;
