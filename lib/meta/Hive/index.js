(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "prettier", "fs-extra", "path", "./HiveMetaGenerator", "../../common/utils", "../MetaLexer"], factory);
    }
    ...
    var HiveMetaGenerator_1 = require("./HiveMetaGenerator");
    ...
    var MetaParserConfig = (function () {
        function MetaParserConfig() {
            ...
            this.parserCodePath = '../../../grammars/HiveSql/CoreParser.g';
            this.lexerCodePath = '../../../grammars/HiveSql/HiveSqlLexer.g';
        }
        return MetaParserConfig;
    }());
    function metaLexerGenerator(lexerGCode, config) {
        var _a = HiveMetaGenerator_1.parseGCode(lexerGCode, false), ast = _a.ast, cst = _a.cst, lexErrors = _a.lexErrors, parseErrors = _a.parseErrors;
        ...
        try {
            ...
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/Hive/hive_lexer_cst.g.json'), JSON.stringify(cst, null, 2));
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/Hive/hive_lexer_ast.g.json'), JSON.stringify(ast, null, 2));
    ...
    function metaParserGenerator(parserGCode, config, tokens) {
        var _a = HiveMetaGenerator_1.parseGCode(parserGCode, true, tokens), ast = _a.ast, cst = _a.cst, lexErrors = _a.lexErrors, parseErrors = _a.parseErrors;
        if (lexErrors && lexErrors.length) {
            console.log.apply(console, lexErrors);
            debugger;
        }
        ...
        var nodes = ast.rules.map(function (rule) {
            ...
            var subRuleNames = [];
            var tokenNames = [];
            HiveMetaGenerator_1.traverse(rule, MetaLexer_1.TokenEnum.LowerName, function (node) {
                subRuleNames.push(node.name);
            });
            HiveMetaGenerator_1.traverse(rule, MetaLexer_1.TokenEnum.UpperName, function (up) {
        ...
        try {
            var beautifiedCode = prettier.format(parserCode, config.prettierConfig);
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/Hive/hive_parser_cst.g.json'), JSON.stringify(cst, null, 2));
            fs.writeFileSync(path.join(__dirname, '../../../sql-parser/Hive/hive_parser_ast.g.json'), JSON.stringify(ast, null, 2));
