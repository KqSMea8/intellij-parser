...
(function (factory) {
    ...
    var peel = function (cst) {
        ...
        return _.flatten(getFilteredNode(query.children[parser_g_1.SyntaxKind.fromClause][0], function (target) { return target.name === parser_g_1.SyntaxKind.tableSources; })[0].children.tableSource.map(function (tableSource) {
            ...
            if (table.children[parser_g_1.SyntaxKind.selectStatement]) {
                tableInfo.exportsFields = Object.values(table.children[parser_g_1.SyntaxKind.selectStatement][0].children).map(function (child) { return peel(child[0]); });
            }
            return tableInfo;
        });
    };
    var getAliasMap = function (ast) {
        ...
        return topSelect.map(function (select) { return peel(select); });
    };
