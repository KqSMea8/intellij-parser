...
(function (factory) {
    ...
    var getPositiondNode = function (cst, pos) {
        return getFilteredNode(cst, function (target) { return target.startLine === pos.line && target.startOffset === pos.offset; });
    };
    ...
    var getFilteredNode = function (cst, filter, global) {
        ...
        var _loop_1 = function () {
            ...
            if (target) {
                queue.push(_.flatten(Object.values(target.children || [])).reduce(function (sum, item) { return sum.concat(__assign({}, item, { ruleStack: target.ruleStack.concat(target) })); }, []));
                if (filter(target)) {
                    if (global) {
                        result.push(target);
                    }
    ...
    var getCompleteInfo = function (ast, pos) {
        ...
        ast.parseErrors.forEach(function (err) {
            ...
            if (errorType.type === definations_1.ErrorToken.singleMissingToken) {
                ...
                Object.keys(definations_1.CommonStartToken).filter(function (item) { return item.toLowerCase().indexOf(err.message.match(errorType.pattern)[1].toLowerCase()) === 0; }).forEach(function (errType) {
                    singleError.completeType.push(__assign({ errType: errType }, err));
                });
            }
            if (errorType.type !== definations_1.ErrorToken.singleMissingToken || err.message.match(errorType.pattern)[1] !== parser_g_1.TokenEnum.SEMI) {
                Object.keys(definations_1.Snippets).forEach(function (errType) {
                    singleError.completeType.push(__assign({ errType: errType }, err));
                });
            }
    ...
    var getTotalPath = function (ast, token) {
        var columnRoot = getFilteredNode(ast.cst, function (target) { return target.name === parser_g_1.SyntaxKind.fullColumnName || target.name === parser_g_1.SyntaxKind.tableName; }, true);
        var targetColumn = _.find(columnRoot, function (o) { return getFilteredNode(o, function (target) { return target.image && target.image.indexOf(token.image) === 0 && target.startColumn === token.startColumn; })[0]; });
        var leafNodes = _.flatten(_.flatten(Object.values(targetColumn.children)).map(function (node) { return getLeafNode(node, true); }));
