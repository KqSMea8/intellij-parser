...
expression:
	...
	| SKIP expression logicalOperator expression
...
predicate:
	SKIP predicate NOT? IN '(' (selectStatement | expressions) ')'
	| SKIP predicate IS nullNotnull
	| left = SKIP predicate comparisonOperator right = predicate
	| SKIP predicate comparisonOperator quantifier = (
		ALL
		| ANY
		| SOME
	) '(' selectStatement ')'
	| SKIP predicate NOT? BETWEEN predicate AND predicate
	| SKIP predicate SOUNDS LIKE predicate
	| SKIP predicate NOT? LIKE predicate (ESCAPE STRING_LITERAL)?
	| SKIP predicate NOT? regex = (REGEXP | RLIKE) predicate
