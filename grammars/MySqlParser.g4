...
createFunction:
	CREATE ownerStatement? FUNCTION fullId '(' functionParameter? (
		',' functionParameter
	)* ')' RETURNS dataType routineOption* routineBody;
...
scheduleExpression:
	...
	| EVERY (decimalLiteral | expression) intervalType (
		...
		ENDS end = timestampValue (endIntervals += intervalExpr)*
	)? # intervalSchedule;
...
singleDeleteStatement:
	DELETE priority = LOW_PRIORITY? QUICK? IGNORE? FROM tableName (
		PARTITION '(' uidList ')'
	)? (WHERE expression)? orderByClause? (LIMIT decimalLiteral)?;
...
queryExpressionUnit: ('(' queryExpression ')')? queryExpressionNointo:
		'(' querySpecificationNointo ')'
		| '(' queryExpressionNointo ')';

	querySpecification:
		SELECT selectSpec* selectElements selectIntoExpression? fromClause? orderByClause?
			limitClause?
	...
	xaStartTransaction:
		XA xaStart = (START | BEGIN) xid xaAction = (
			JOIN
			| RESUME
		)?;
	...
	alterUser:
		...
		| ALTER USER ifExists? userAuthOption (
			',' userAuthOption
		)* (
			REQUIRE (
				tlsNone = NONE
	...
	userAuthOption:
		...
		| userName IDENTIFIED (WITH authPlugin)? BY STRING_LITERAL	# stringAuthOption
		| userName IDENTIFIED WITH authPlugin (AS STRING_LITERAL)?	# hashAuthOption
	...
	optimizeTable:
		OPTIMIZE actionOption = (NO_WRITE_TO_BINLOG | LOCAL)? TABLE tables;
	...
	setStatement:
		...
		| SET NAMES (
			charsetName (COLLATE collationName)?
			| DEFAULT
		)							# setNames
	...
	killStatement:
		KILL connectionFormat = (CONNECTION | QUERY)? decimalLiteral+;
	...
	specificFunction: (
		...
		| GET_FORMAT '(' datetimeFormat = (
			DATE
			| TIME
			| DATETIME
		) ',' stringLiteral ')' # getFormatFunctionCall;
	...
	predicate:
		...
		| SKIP predicate NOT? LIKE predicate (
